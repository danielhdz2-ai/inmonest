...
export default function MapSearchView({ listings }: Props) {
  ...
  return (
    <div className="flex h-[calc(100vh-200px)] gap-3 min-h-[500px]">
      {/* Panel izquierdo: lista compacta */}
      <div
        ref={listRef}
        className="w-80 xl:w-96 shrink-0 overflow-y-auto flex flex-col gap-2 pr-1"
      >
        {listings.length === 0 ? (
          <div className="text-center py-16 text-gray-400 text-sm">No hay resultados con filtros actuales</div>
        ) : (
          listings.map((listing) => {
            ...
            return (
              <Link
                ...
                className={`flex gap-0 bg-white rounded-xl overflow-hidden border transition-all hover:shadow-md ${
                  isActive
                    ? 'border-[#c9962a] shadow-[#c9962a]/20 shadow-md ring-1 ring-[#c9962a]/30'
                    : 'border-gray-100 hover:border-gray-200'
                }`}
                onMouseEnter={() => {
                  if (hasCoords) setActiveId(listing.id)
                }}
                onMouseLeave={() => setActiveId(null)}
              >
                {/* Imagen */}
                <div className="w-24 h-[90px] shrink-0 bg-gray-100 overflow-hidden">
                  {imgUrl ? (
                ...
                <div className="flex-1 px-3 py-2.5 min-w-0">
                  ...
                  <div className="flex gap-2 mt-1.5 text-xs text-gray-400 flex-wrap">
                    {listing.bedrooms != null && (
                      <span>🛏️ {listing.bedrooms === 0 ? 'Estudio' : listing.bedrooms}</span>
                    )}
        ...
        {listings.length > 0 && (
          <p className="text-xs text-gray-400 text-center py-2">
            {withCoords.length} de {listings.length} con ubicación en el mapa
          </p>
        )}
      </div>
      ...
      <div
        ref={mapElRef}
        className="flex-1 rounded-xl overflow-hidden border border-gray-100 shadow-sm"
        style={{ minHeight: 400 }}
      />
    </div>
  )
