...
import {
  getTramiteOnlineSectionContent,
  type TramiteOnlineSectionTipo,
} from '@/lib/gestoria-tramite-online-section-content'

type Props = {
  ciudad: string
  /** Ancla del panel interactivo en la misma página */
  panelAnchorId?: string
  /** Copy del bloque según tipo de contrato */
  tipo?: TramiteOnlineSectionTipo
}

export default function GestoriaTramiteOnlineSection({
  ciudad,
  panelAnchorId = 'panel-gestoria-lau',
  tipo = 'lau',
}: Props) {
  const content = getTramiteOnlineSectionContent(tipo, ciudad)
    ...
    >
      <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-14">
        <div>
          ...
          <p className="mt-4 text-gray-600 leading-relaxed">{content.hero}</p>
          <p className="mt-3 text-sm text-gray-600 leading-relaxed">{content.desc}</p>
        ...
        <div className="relative aspect-[4/3] overflow-hidden rounded-2xl shadow-xl ring-1 ring-black/5">
          ...
          <div className="absolute bottom-4 left-4 right-4 rounded-xl bg-white/95 p-4 shadow-lg backdrop-blur-sm">
            ...
            <p className="text-sm font-bold text-gray-900">{content.overlaySubtitulo}</p>
          </div>
        </div>
      </div>
    </section>
