  ...
  async function handleClick() {
    ...
    try {
      const res = await fetch('/api/gestoria/checkout', {
        ...
        body: JSON.stringify({ service_key: 'arras-penitenciales' }),
      })
      ...
      >
        {loading ? (
          ...
          <>
            ...
            Revisión por gestoría — 120 €
          </>
        )}
      </button>
      {error && <p className="text-red-600 text-sm">{error}</p>}
      <p className="text-xs text-gray-500">Pago seguro con Stripe · Revisión en 48h · Abogado especialista en {ciudad}</p>
    </div>
  )
}
